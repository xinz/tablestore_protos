...
defmodule ExAliyunOts.TableStore.StartLocalTransactionRequest do
  ...
  (
    ...
    [
      ...
      defp encode_key(acc, msg) do
        try do
          case msg.key do
            nil -> raise Protox.RequiredFieldsError.new([:key])
            _ -> [acc, "\x12", Protox.Encode.encode_bytes(msg.key)]
          end
        rescue
          ArgumentError ->
        ...
        {new_set_fields, field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {2, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[:key | set_fields], [key: delimited], rest}
  ...
  (
    ...
    def json_decode!(input, opts \\ []) do
      ...
      Protox.JsonDecode.decode!(
        input,
        ExAliyunOts.TableStore.StartLocalTransactionRequest,
  ...
  (
    ...
    def defs() do
      %{1 => {:table_name, {:scalar, ""}, :string}, 2 => {:key, {:scalar, ""}, :bytes}}
    end
    ...
    def defs_by_name() do
      %{key: {2, {:scalar, ""}, :bytes}, table_name: {1, {:scalar, ""}, :string}}
    end
  )
