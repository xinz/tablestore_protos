...
defmodule(ExAliyunOts.TableStoreSearch.TermsQuery) do
    ...
    [
      ...
      defp(encode_terms(acc, msg)) do
        try do
          ...
          ArgumentError ->
            reraise(Protox.EncodingError.new(:terms, "invalid field value"), __STACKTRACE__)
        end
      end
    ]
        ...
        {field, rest} =
          case(Protox.Decode.parse_key(bytes)) do
            ...
            {2, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[terms: msg.terms ++ [delimited]], rest}
  ...
  (
    ...
    def(json_decode!(input, opts \\ [])) do
      ...
      Protox.JsonDecode.decode!(
        input,
        ExAliyunOts.TableStoreSearch.TermsQuery,
  ...
  (
    ...
    def(defs()) do
      %{1 => {:field_name, {:scalar, ""}, :string}, 2 => {:terms, :unpacked, :bytes}}
    end
    ...
    def(defs_by_name()) do
      %{field_name: {1, {:scalar, ""}, :string}, terms: {2, :unpacked, :bytes}}
    end
  )
