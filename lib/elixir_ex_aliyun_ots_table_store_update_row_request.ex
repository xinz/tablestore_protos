      ...
      defp encode_row_change(acc, msg) do
        try do
          case msg.row_change do
            nil -> raise Protox.RequiredFieldsError.new([:row_change])
            _ -> [acc, "\x12", Protox.Encode.encode_bytes(msg.row_change)]
          end
        rescue
          ArgumentError ->
      ...
      defp encode_condition(acc, msg) do
        try do
          case msg.condition do
            nil -> raise Protox.RequiredFieldsError.new([:condition])
            _ -> [acc, "\x1A", Protox.Encode.encode_message(msg.condition)]
          end
        rescue
          ArgumentError ->
      ...
      defp encode_return_content(acc, msg) do
        try do
          case msg.return_content do
            nil -> acc
            _ -> [acc, "\"", Protox.Encode.encode_message(msg.return_content)]
          end
        rescue
          ArgumentError ->
            reraise Protox.EncodingError.new(:return_content, "invalid field value"),
                    __STACKTRACE__
        end
      end,
      defp encode_transaction_id(acc, msg) do
        try do
          case msg.transaction_id do
            nil -> acc
            _ -> [acc, "*", Protox.Encode.encode_string(msg.transaction_id)]
          end
        rescue
          ArgumentError ->
  ...
  (
    ...
    def json_decode!(input, opts \\ []) do
      ...
      Protox.JsonDecode.decode!(
        input,
        ExAliyunOts.TableStore.UpdateRowRequest,
