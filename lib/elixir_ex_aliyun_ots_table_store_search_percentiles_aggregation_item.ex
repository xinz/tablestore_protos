    ...
    [
      defp encode_key(acc, msg) do
        try do
          case msg.key do
            nil -> acc
            _ -> [acc, "\t", Protox.Encode.encode_double(msg.key)]
          end
        rescue
          ArgumentError ->
      ...
      defp encode_value(acc, msg) do
        try do
          case msg.value do
            nil -> acc
            _ -> [acc, "\x12", Protox.Encode.encode_bytes(msg.value)]
          end
        rescue
          ArgumentError ->
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(ExAliyunOts.TableStoreSearch.PercentilesAggregationItem))
        end
      )
    )
