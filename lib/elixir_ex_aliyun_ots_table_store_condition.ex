    ...
    [
      defp encode_row_existence(acc, msg) do
        try do
          ...
          ArgumentError ->
            reraise Protox.EncodingError.new(:row_existence, "invalid field value"),
                    __STACKTRACE__
        end
      end,
      defp encode_column_condition(acc, msg) do
        try do
          case msg.column_condition do
            nil -> acc
            _ -> [acc, "\x12", Protox.Encode.encode_bytes(msg.column_condition)]
          end
        rescue
          ArgumentError ->
            reraise Protox.EncodingError.new(:column_condition, "invalid field value"),
                    __STACKTRACE__
        end
      end
    ]
